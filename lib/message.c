		    ...
		    struct const_buffer *buf)
{
	...
	ma->addrs = parr_new(vlen, bitc_addr_freep);
	if (!ma->addrs)
		return false;
...
bool deser_msg_headers(struct msg_headers *mh, struct const_buffer *buf)
{
	...
	mh->headers = parr_new(vlen, bitc_block_freep);
...
bool deser_msg_version(struct msg_version *mv, struct const_buffer *buf)
{
	...
	if (!deser_bitc_addr(INIT_PROTO_VERSION, &mv->addrTo, buf)) return false;

	if (mv->nVersion >= 106) {
		if (!deser_bitc_addr(INIT_PROTO_VERSION, &mv->addrFrom, buf)) return false;
...
cstring *ser_msg_version(const struct msg_version *mv)
{
	...
	ser_bitc_addr(s, INIT_PROTO_VERSION, &mv->addrTo);
	ser_bitc_addr(s, INIT_PROTO_VERSION, &mv->addrFrom);
...
bool deser_msg_vinv(struct msg_vinv *mv, struct const_buffer *buf)
{
	...
	mv->invs = parr_new(vlen, bitc_inv_freep);
...
{
	if (!mv->invs)
		mv->invs = parr_new(512, bitc_inv_freep);
